...
export function list<T extends Data.Value>(type: Meta.Type<Data.List<T>>, members: T[]): Data.List<T> {
...
export function record<F extends Data.FieldValues>(type: Meta.Type<Data.Record<F>>, members: F): Data.Record<F> {
...
export function tuple<T extends Data.ValueSequence>(type: Meta.Type<Data.Tuple<T>>, members: T): Data.Tuple<T> {
  ...
  implements Data.CompositeValue<Ix, T, C, Shadow>
{
  ...
  constructor(type: Meta.Type<C>, shadow: Shadow) {
    this.#type = type
    this.#shadow = shadow
  }
  get type(): Meta.Type<C> {
    return this.#type
  }
  get shadow(): Shadow {
    return this.#shadow
  ...
  abstract get associations(): IteratorObject<[Ix, T]>
...
const listElementType: Meta.TypePattern<Meta.Type<Data.Value>, []> = {
...
class DictionaryValue<T extends Data.Value> extends CompositeValue<string, T, Data.Dictionary<T>, Data.Table<T>> {
  ...
  constructor(type: Meta.Type<Data.Dictionary<T>>, shadow: Data.Table<T>, size: number) {
    super(type, shadow)
    this.#size = size
  }
  get size() {
...
const dictionaryElementType: Meta.TypePattern<Meta.Type<Data.Value>, []> = {
...
class RecordValue<F extends Data.FieldValues> extends CompositeValue<string, F[keyof F], Data.Record<F>, F> {
  readonly #size: number
  constructor(type: Meta.Type<Data.Record<F>>, shadow: F, size: number) {
    super(type, shadow)
    this.#size = size
  }
  get size() {
