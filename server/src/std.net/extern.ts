...
export default async ({ use, former }: Contract<"std.net">): Promise<Service["std.net"]> => {
  const standardContractor = former as () => Promise<Service["std.net"]>
  const standardProvider = await standardContractor()
  use
  ...
  //     "std.theater"
  //   )
  return { ...standardProvider, ...(await import("./intern.js")) }
}
